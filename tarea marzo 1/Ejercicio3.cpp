...
using namespace std;
// se crea la estructura para los nodos
struct nodo{
     int nro; 
     struct nodo *izq, *der; // se crea los punteros
};

 typedef struct nodo *Arbol; //se crea el punto pára el nodo de los arboles

Arbol crearNodo(int x){
     Arbol nuevoNodo = new(struct nodo);
     nuevoNodo->nro = x; // indicamos el arbol que se direcciona
     nuevoNodo->izq = NULL; 
     nuevoNodo->der = NULL;
...
void insertar(Arbol &arbol, int x){
     if(arbol==NULL){
           arbol = crearNodo(x);
     } // si el arbol es null se crea el arbol y en la variable x se ingresaran los nodos 
     else if(x < arbol->nro)  // de lo contrario se inserta en el nodo derecho
...
// se crea el void que es la funcion la cual ingresa los datos
void ingresardatos(){
    //Arbol arbol = NULL; +
